...
public final class Administrador extends javax.swing.JFrame {
    ...
    public void actualizarTablaMarcas() {
        marcaBO.actualizarTabla((DefaultTableModel) tblMarca.getModel());
    }

    public void actualizarTablaProveedores() {
        proveedorBO.actualizarTabla((DefaultTableModel) tblProveedores.getModel());
    }

    public void actualizarTablaDetalle() {
        detalleCompraBO.actualizarTabla((DefaultTableModel) tblDetalleCompra.getModel());
    }

    public void actualizarTablaCompra() {
        compraBO.actualizarTabla((DefaultTableModel) tblCompra.getModel());
    }
    ...
    private void btnGuardarProveedorActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnGuardarProveedorActionPerformed
        ...
        // Crear la instancia de Proveedor con los datos ingresados
        Proveedor nuevoProveedor = new Proveedor(
                idmanager.generarID("proveedor.json") + 1, // Genera el ID para el proveedor
                cedula,
                nombre,
                telefono,
                correo
        ...
        // Crear instancia de ProveedorBO para guardar el nuevo proveedor
        ProveedorBO proveedorBO = new ProveedorBO();
        try {
            proveedorBO.guardarProveedor(nuevoProveedor);
        } catch (IOException | ParseException e) {
            JOptionPane.showMessageDialog(this, "Ocurrió un error al guardar el proveedor.", "Error", JOptionPane.ERROR_MESSAGE);
            e.printStackTrace();
        }

        // Actualizar la tabla de proveedores y limpiar campos
        actualizarTablaProveedores();
        limpiarCampos();
    ...
    private void btnEliminarProveedorActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnEliminarProveedorActionPerformed
        ...
        if (selectedRow != -1) {
            ...
            // Crear una instancia de ProveedorBO
            ProveedorBO proveedorBO = new ProveedorBO();

            try {
                // Llamar al método eliminarProveedor en ProveedorBO
                proveedorBO.eliminarProveedor(idProveedorAEliminar);

                // Actualizar la tabla de proveedores y limpiar campos
                actualizarTablaProveedores();
                limpiarCampos();
            } catch (IOException | ParseException e) {
                JOptionPane.showMessageDialog(this, "Ocurrió un error al eliminar el proveedor.", "Error", JOptionPane.ERROR_MESSAGE);
                e.printStackTrace();
            }
        }
    ...
    private void btnEditarProveedorActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnEditarProveedorActionPerformed
        ...
        if (selectedRow != -1) {
            ...
            // Obtener el ID del proveedor a partir de la tabla
            int idProveedor = Integer.parseInt(tblProveedores.getValueAt(selectedRow, 0).toString());

            // Crear una instancia de Proveedor con los datos actualizados
            Proveedor proveedorActualizado = new Proveedor(idProveedor, cedula, nombre, telefono, correo);

            // Crear instancia de ProveedorBO
            ProveedorBO proveedorBO = new ProveedorBO();

            try {
                // Llamar al método editarProveedor en ProveedorBO
                proveedorBO.editarProveedor(proveedorActualizado);
                ...
                JOptionPane.showMessageDialog(this, "Ocurrió un error al actualizar el proveedor.", "Error", JOptionPane.ERROR_MESSAGE);
                e.printStackTrace();
            }
        } else {
...
// Limpiar los campos de entrada
    ...
    private void btnEditarMarcaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnEditarMarcaActionPerformed
        ...
        if (selectedRow >= 0) {
            ...
            // Obtener el ID de la marca desde la tabla
            int idMarca = Integer.parseInt(tblMarca.getValueAt(selectedRow, 0).toString());

            // Crear instancia de MarcaBO
            MarcaBO marcaBO = new MarcaBO();

            // Llamar al método editarMarca en MarcaBO
            marcaBO.editarMarca(idMarca, nombre);

            // Actualizar la tabla de marcas y limpiar campos
            actualizarTablaMarcas();
            limpiarCampos();
            JOptionPane.showMessageDialog(null, "Se actualizó la marca");
        } else {
            JOptionPane.showMessageDialog(null, "Selecciona una marca en la tabla");
        }
...
// Crear una instancia de Marca con los datos ingresados
        Marca nuevaMarca = new Marca(
                idmanager.generarID("marca.json") + 1, // Genera el ID para la marca
                nombre
        );
...
// Crear instancia de MarcaBO
        ...
        // Guardar la nueva marca a través de MarcaBO
        marcaBO.guardarMarca(nuevaMarca);
        // Actualizar la tabla de marcas
        actualizarTablaMarcas();
        // Limpiar los campos de entrada
        limpiarCampos();
    ...
    private void btnEditarDetalleActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnEditarDetalleActionPerformed
        ...
        if (selectedRow != -1) {
            ...
            // Crear instancia de DetalleCompraBO
            DetalleCompraBO detalleCompraBO = new DetalleCompraBO();

            // Llamar al método editarDetalleCompra en DetalleCompraBO
            detalleCompraBO.editarDetalleCompra(idDetalle, cantidad, monto, idProductoDetalle, idCompraDetalle);
            // Actualizar la tabla de detalles de compra y limpiar campos
    ...
    private void btnEliminarDetalleActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnEliminarDetalleActionPerformed
        ...
        if (selectedRow != -1) {
            ...
            // Llamar al método eliminarDetalleCompra en DetalleCompraBO
            detalleCompraBO.eliminarDetalleCompra(idDetalleCompraAEliminar);
            // Actualizar la tabla de detalles de compra y limpiar campos
            actualizarTablaDetalle();
            limpiarCampos();
    ...
    private void btnEditarCompraActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnEditarCompraActionPerformed
        ...
        if (selectedRow != -1) {
            ...
            // Crear instancia de CompraBO
            CompraBO compraBO = new CompraBO();

            // Llamar al método editarCompra en CompraBO
            compraBO.editarCompra(idCompra, fecha, montoTotal, idCliente, idDetalleCompra);
            // Actualizar la tabla de compras y limpiar campos
            actualizarTablaCompra();
            limpiarCampos();
    ...
    private void btnEliminarCompraActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnEliminarCompraActionPerformed
        ...
        if (selectedRow != -1) {
            ...
            // Crear instancia de CompraBO
            CompraBO compraBO = new CompraBO();
            // Llamar al método eliminarCompra en CompraBO
            compraBO.eliminarCompra(idCompraAEliminar);
            // Actualizar la tabla de compras y limpiar campos
            actualizarTablaCompra();
            limpiarCampos();
